...
async def start(msg: types.Message):
    '''Помощь и инструкции'''
    text = '''Команда /start доступна только админу так что жмать не надо.
Команда /add записывает Вас в бд бота и далее следуйте инструкциям:
1) Отправьте в формате ДД.ММ.ГГГГ или ДД.ММ свой др обязательно реплаем/ответов на сообщение бота,
если просто отправить в чат, то бот не обратит внимание
2) Точно так же напишите свой пол.
Готово. В 7 утра в Ваш др он сообщит всем о Вашем дне рождения.
Функционал корректировки даты будет добавлен потом. Если чё пишите админу.
'''
